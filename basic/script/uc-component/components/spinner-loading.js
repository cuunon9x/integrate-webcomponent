...
const SpinnerLoading = /*@__PURE__*/ proxyCustomElement(class SpinnerLoading extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
  ...
  static get style() { return spinnerLoadingCss; }
}, [1, "uc-spinner-loading"]);
function defineCustomElement() {
  if (typeof customElements === "undefined") {
    return;
  }
  const components = ["uc-spinner-loading"];
  components.forEach(tagName => { switch (tagName) {
    case "uc-spinner-loading":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, SpinnerLoading);
      }
      break;
  } });
}

export { SpinnerLoading as S, defineCustomElement as d };
